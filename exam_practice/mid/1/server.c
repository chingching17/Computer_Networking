...
void handle_message(char inputbuf[]){
    ...
    if(strcmp(cmd, "HISTORY") == 0){
        if(cli_account.his_count < 0){
            ...
            for(int i = 0 ; i <= cli_account.his_count ; i++){
                ...
                sprintf(response,"%s",cli_account.history[i]);
                send(cli_account.sockfd, response, sizeof(response), 0);
    ...
    if(strcmp(cmd,"WITHDRAW") == 0){
        ...
        if(now_balance > 0){
            ...
            sprintf(his2,"### %s\n",inputbuf);
            // char * str_his2 = his2;
            // cli_account.history[cli_account.his_count] = str_his2;
            cli_account.history[cli_account.his_count] = \
                (char *)malloc(sizeof(char)* (strlen(his2)+1));
            strcpy(cli_account.history[cli_account.his_count], his2);
            sprintf(response,"### BALANCE: %d NTD\n",cli_account.balance);
            send(cli_account.sockfd, response, sizeof(response), 0);
            return;
            ...
            // char * str_his = his;
            // cli_account.history[cli_account.his_count] = str_his;
            
            cli_account.history[cli_account.his_count] = \
                (char *)malloc(sizeof(char)* (strlen(his)+1));
            strcpy(cli_account.history[cli_account.his_count], his);
    ...
    if(strcmp(cmd,"DEPOSIT") == 0){
        ...
        if(now_balance_d > 0){
            ...
            sprintf(his3,"### %s\n",inputbuf);
            // char * str_his3 = his3;
            // cli_account.history[cli_account.his_count] = str_his3;
            
            cli_account.history[cli_account.his_count] = \
                (char *)malloc(sizeof(char)* (strlen(his3)+1));
            strcpy(cli_account.history[cli_account.his_count], his3);
